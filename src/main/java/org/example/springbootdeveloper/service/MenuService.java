...
public class MenuService {
    ...
    public ResponseDto<MenuResponseDto> getMenuById(Long id) {
        ...
        try {
                //Optional
                // null 값을 처리하는 방법
                // <Generic> 타입의 구조를 선택적으로 받아오는 클래스
                // : 데이터가 있을 수도 있고 없을 수도 있는 경우에 사용

                // 만약 해당하는 데이터가 있을 경우 Optional 안에 데이터 객체를 담고 (찾을 땐 .get() 사용)
    ...
    public ResponseDto<MenuResponseDto> updateMenu(Long id, @Valid MenuRequestDto dto) {
        MenuResponseDto data = null;
        Long menuId = id;
        try {
            Optional<Menu> menuOptional = menuRepository.findById(menuId);
