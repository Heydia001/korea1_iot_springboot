...
class TestControllerTest {
    ...
    public void getAllmembers() throws Exception {
        ...
        // - 결과를 ResultAction 객체로 받음
        // - ResultAction 객체는 반환값을 검증하고 확인하는  andExpect() 메서드를 제공
        final ResultActions result = mockMvc.perform(get(url)
                // .accept(): 요청을 받을 때 무슨 타입으로 응답을 받을 지 결정하는 메서드
                // - JSON, XML 등이 있음
        ...
        result
                ...
                // - .isOk() 메서드로 응답 코드가 맞는지 확인
                .andExpect(status().isOk())
                // JsonPath("$[0].필드명")
                // : JSON의 응답값을 가져오는 역할을 담당
                // : 0번째 배열에 들어 있는 객체의 id, name을 가져오고 저장된 값과 같은지 확인
...
// 400번대 응답 코드 - id4xxClientError()
// 500 internal Serval Error - is InternalServerError()
